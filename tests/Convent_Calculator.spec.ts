...
test.describe('Conventional Calculator', () => {
    let page: Page;
    ...
    test("All Data Valid", async () => {        
        ...
        await expect(result).toHaveText('Rp 543.235.962') // Result

    })
    test("Invalid 'Interest Rate'", async () => {        

        console.log("Fill Calculator Data");
        await calculator.fillKPRType('conventional'); //'conventional' or 'syariah'
        await calculator.fillTenor('10') 
        await calculator.fillInterestRate('asd') 
        await calculator.fillSalary('10000000')  
        await calculator.fillOngoingInstallment('0') 
        await calculator.clickCalculateButton()
